...
export function getMonitorAttachment(key: MonitorKey): MonitorAttachment | undefined {
  if (key === 'none') return { layout: 'none', panelWidthCm: 0, panelDepthCm: 0 };
  const gap = 2; // cm per gap for triples
  switch (key) {
    case 'single-49': {
      // 49" ultrawide ~ 119cm width, depth ~ 30cm
      return { layout: 'single', screenInches: 49, panelWidthCm: 119, panelDepthCm: 30 };
    }
    case 'triple-42': {
      // 42" ~ 93cm wide per panel, depth ~ 30cm
      const w = 93;
      return { layout: 'triple', screenInches: 42, panelWidthCm: w * 3 + gap * 2, panelDepthCm: 30 + 10, gapCm: gap, angleDeg: 60 };
    }
    case 'triple-45c': {
      // 45" curved ~ 105cm per panel, depth ~ 32cm
      const w = 105;
      return { layout: 'triple', screenInches: 45, panelWidthCm: w * 3 + gap * 2, panelDepthCm: 32 + 10, gapCm: gap, angleDeg: 60 };
    }
    case 'triple-55': {
      const w = 121;
      return { layout: 'triple', screenInches: 55, panelWidthCm: w * 3 + gap * 2, panelDepthCm: 35 + 10, gapCm: gap, angleDeg: 60 };
    }
    case 'triple-65': {
      const w = 144;
      return { layout: 'triple', screenInches: 65, panelWidthCm: w * 3 + gap * 2, panelDepthCm: 40 + 10, gapCm: gap, angleDeg: 60 };
    }
    default:
      return undefined;
  }
