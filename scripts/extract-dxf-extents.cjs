/* Simple DXF extent extractor for LINE entities on a given layer (default: "0")
   Usage: node scripts/extract-dxf-extents.cjs "/absolute/path/to/file.dxf" [layerName]
...
function parseDxfExtents(filePath, targetLayer = '0') {
  ...
  if (!isFinite(minX) || !isFinite(minY) || !isFinite(maxX) || !isFinite(maxY)) {
    const layersObject = Object.fromEntries([...layerCounts.entries()].sort((a, b) => b[1] - a[1]));
    return {
      ok: false,
...
function main() {
  ...
  if (!filePath) {
    console.error('Usage: node scripts/extract-dxf-extents.cjs \"/absolute/path/to/file.dxf\" [layerName]');
    process.exit(1);
  }
